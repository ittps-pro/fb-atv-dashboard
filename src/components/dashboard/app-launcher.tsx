    ...
    <Card className="bg-card/50 backdrop-blur-sm h-full">
      ...
      <CardContent>
        <div className="grid grid-cols-3 gap-4">
          {visibleApps.map((app) => {
            const Icon = iconMap[app.iconName as IconName] || AppWindow;
            return (
              <Card 
                key={app.name}
              ...
              >
                  <div className="p-3 bg-secondary rounded-xl transition-all duration-300 group-hover:scale-110 group-hover:bg-accent/30">
                    <Icon className="h-8 w-8 text-accent transition-colors duration-300 group-hover:text-primary-foreground" />
                  </div>
