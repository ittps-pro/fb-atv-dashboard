...
export async function POST(request: Request) {
  ...
  try {
    ...
    console.log(`SIMULATING MOUNT for storage: ${storage.name} (${storage.protocol})`);
    ...
    await withFileLock(async () => {
        ...
        if (currentStorage) {
            // In a real scenario, check if the mount command succeeded
            // For demo, we'll assume it succeeds.
            currentStorage.status = 'mounted';
            await saveStorages(currentStorages);
        }
    });
    
    return NextResponse.json({ success: true, message: `Successfully mounted ${storage.name}.` });

  } catch(error: any) {
    console.error('Storage mount API error:', error.message);
    await withFileLock(async () => {
        const storages = await getStorages();
    ...
    return NextResponse.json({ error: 'Failed to process mount request.', details: error.message }, { status: 500 });
  }
}
